...
class ErrorCode(Enum):
    ...
    BACKTEST_RESULT_ERROR = ErrorCodeInfo(
        "BT_003", "backtest", "Failed to process backtest results"
    )

    # Optimization errors (OPT_xxx)
    OPTIMIZATION_ERROR = ErrorCodeInfo("OPT_001", "optimization", "Optimization failed")
    OPTIMIZATION_CONVERGENCE = ErrorCodeInfo(
        "OPT_002", "optimization", "Optimization did not converge"
    )
    OPTIMIZATION_INVALID_PARAMS = ErrorCodeInfo(
        "OPT_003", "optimization", "Invalid optimization parameters"
    )

    # Metrics errors (MET_xxx)
    METRICS_CALCULATION_ERROR = ErrorCodeInfo("MET_001", "metrics", "Failed to calculate metrics")
    METRICS_INVALID_INPUT = ErrorCodeInfo(
        "MET_002", "metrics", "Invalid input for metrics calculation"
    )
