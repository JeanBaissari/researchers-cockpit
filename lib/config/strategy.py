...
def load_strategy_params(strategy_name: str, asset_class: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not params_path.exists():
        raise FileNotFoundError(
            f"parameters.yaml not found for strategy '{strategy_name}'. Expected at: {params_path}"
        )
...
def get_warmup_days(params: Dict[str, Any]) -> int:
    ...
    if period_values:
        ...
        logger.debug(
            f"Calculated warmup_days from periods: {max_period} (from {len(period_values)} period params)"
        )
        return max_period
