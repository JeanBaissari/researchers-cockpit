...
def _plot_optimization_heatmap(
    results_df: pd.DataFrame, param_grid: dict, objective: str, save_dir: Path
) -> None:
    """
    ...
    pivot_data = results_df.pivot_table(
        values=test_obj_col, index=param_names[0], columns=param_names[1], aggfunc="mean"
    )
...
def _plot_monte_carlo_distribution(simulation_results: dict, save_dir: Path) -> None:
    """
    ...
    ax.set_title(
        "Monte Carlo Simulation - Final Value Distribution", fontsize=14, fontweight="bold"
    )
    ax.legend()
