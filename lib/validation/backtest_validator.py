...
class BacktestValidator(BaseValidator):
    ...
    def validate(
        ...
        if positions is not None and transactions is not None:
            result = self._run_check(
                result, self._check_positions_transactions, positions, transactions
            )

        return result

    def _check_metric_ranges(
        self, result: ValidationResult, metrics: Dict[str, Any], returns: Optional[pd.Series]
    ) -> ValidationResult:
        """Wrapper for _check_metrics."""
        return self._check_metrics(result, metrics, returns)

    def _check_return_consistency(
        self, result: ValidationResult, returns: pd.Series, transactions: Optional[pd.DataFrame]
    ) -> ValidationResult:
        """Wrapper for _check_returns."""
        return self._check_returns(result, returns, transactions)

    def _check_position_consistency(
        self, result: ValidationResult, positions: pd.DataFrame, transactions: pd.DataFrame
    ) -> ValidationResult:
    ...
    def _check_metrics(
        ...
        if sharpe is not None:
            if not -10 <= sharpe <= 10:
                ...
                result.add_check(
                    "sharpe_range", True, "Sharpe ratio within expected range", {"sharpe": sharpe}
                )
        ...
        if win_rate is not None:
            if not 0 <= win_rate <= 1:
                ...
                result.add_check(
                    "win_rate_range", True, "Win rate within valid range", {"win_rate": win_rate}
                )
    ...
    def _check_returns(
        ...
        if len(extreme) > 0:
            ...
            result.add_check("extreme_returns", True, "No extreme daily returns")
