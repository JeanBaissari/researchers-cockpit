...
DAILY_TIMEFRAMES: FrozenSet[str] = frozenset({"1d", "daily", "1wk", "weekly", "1mo", "monthly"})
ALL_TIMEFRAMES: FrozenSet[str] = INTRADAY_TIMEFRAMES | DAILY_TIMEFRAMES

# Expected column names (lowercase canonical form)
REQUIRED_OHLCV_COLUMNS: FrozenSet[str] = frozenset(["open", "high", "low", "close", "volume"])
OPTIONAL_OHLCV_COLUMNS: FrozenSet[str] = frozenset(["adj_close", "dividends", "splits", "vwap"])
...
CONTINUOUS_CALENDARS: FrozenSet[str] = frozenset({"24/7", "FOREX", "24_7", "CRYPTO", "always_open"})
