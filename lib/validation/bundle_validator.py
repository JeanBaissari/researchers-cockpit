...
class BundleValidator(BaseValidator):
    ...
    def validate(self, bundle_name: str, bundle_path: Optional[Path] = None) -> ValidationResult:
        """
        ...
        if not bundle_path or not bundle_path.exists():
            result.add_check("bundle_exists", False, f"Bundle path does not exist: {bundle_path}")
            return result

        result.add_check("bundle_exists", True, f"Bundle '{bundle_name}' exists at {bundle_path}")
    ...
    def _check_bundle_exists(self, result: ValidationResult, bundle_path: Path) -> ValidationResult:
        """Check bundle directory exists (handled in validate_bundle)."""
        return result

    def _check_bundle_metadata(
        self, result: ValidationResult, bundle_path: Path
    ) -> ValidationResult:
    ...
    def _check_bundle_assets(self, result: ValidationResult, bundle_path: Path) -> ValidationResult:
