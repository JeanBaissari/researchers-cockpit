...
def calculate_trade_metrics(
    transactions: pd.DataFrame, as_percentages: bool = False
) -> Dict[str, float]:
    """
    ...
    avg_trade_duration = (
        sanitize_value(float(np.mean(trade_durations))) if len(trade_durations) > 0 else 0.0
    )
            ...
            if current_position["quantity"] == 0:
                ...
                if close_quantity >= current_position["quantity"]:
                    ...
                    current_position["total_cost"] = (
                        current_position["quantity"] * current_position["weighted_avg_price"]
                    )
