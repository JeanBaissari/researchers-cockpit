                ...
                transactions_list.append(
                    {
                        ...
                        if txn.get("commission") is not None
                        else 0.0,
...
def save_performance_dataframe(perf: pd.DataFrame, result_dir: Path) -> None:
    """
    Save full performance DataFrame as pickle for later loading.

    This preserves all Zipline native metrics (alpha, beta, leverage, etc.)
    that are not available in CSV exports.

    Args:
        perf: Performance DataFrame from Zipline
        result_dir: Directory to save pickle file
    """
    try:
        import pickle

        perf_file = result_dir / "performance.pkl"
        with open(perf_file, "wb") as f:
            pickle.dump(perf, f)
        logger.debug(f"Saved performance DataFrame to {perf_file}")
    except Exception as e:
        logger.warning(f"Could not save performance DataFrame: {e}")
...
def save_transactions_csv(transactions_df: pd.DataFrame, result_dir: Path) -> None:
    ...
    transactions_df.to_csv(
        result_dir / "transactions.csv", date_format="%Y-%m-%d", index_label="date"
    )
    ...
    for date in dates:
        ...
        if date_only in transactions_by_date.groups:
            ...
            for _, txn in date_transactions.iterrows():
                ...
                commission = (
                    float(txn.get("commission", 0.0)) if pd.notna(txn.get("commission")) else 0.0
                )
        ...
        if date_only in positions_by_date.groups:
            ...
            for _, pos in date_positions.iterrows():
                ...
                last_sale_price = (
                    float(pos.get("last_sale_price", 0.0))
                    if pd.notna(pos.get("last_sale_price"))
                    else 0.0
                )
                if last_sale_price > 0:
                    ...
                    cost_basis = (
                        float(pos.get("cost_basis", 0.0))
                        if pd.notna(pos.get("cost_basis"))
                        else 0.0
                    )
...
def calculate_and_save_metrics(
    ...
    if "returns" in perf.columns:
        ...
        logger.info(
            "Reconstructing portfolio_value from transactions and positions (metrics_set='none' was used)"
        )
        ...
        if len(portfolio_value) > 1:
            ...
            logger.info(
                f"Reconstructed portfolio_value and calculated returns from {len(portfolio_value)} data points"
            )
        else:
    ...
    if portfolio_value is not None and len(portfolio_value) > 0:
        ...
        if "portfolio_value" not in perf.columns:
            perf["portfolio_value"] = portfolio_value.reindex(perf.index, method="ffill").fillna(
                initial_capital if initial_capital else 100000.0
            )
