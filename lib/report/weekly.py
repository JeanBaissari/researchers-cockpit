...
def _build_weekly_summary(
    strategies: List[Dict[str, Any]], start_date: Optional[str], end_date: Optional[str]
) -> str:
    ...
    strategies_sorted = sorted(
        strategies, key=lambda x: x["metrics"].get("sharpe", 0), reverse=True
    )
...
def _build_summary_statistics(
    strategies: List[Dict[str, Any]], strategies_sorted: List[Dict[str, Any]]
) -> str:
