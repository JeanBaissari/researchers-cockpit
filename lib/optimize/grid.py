...
def grid_search(
    ...
    for i, combo in enumerate(combinations):
        ...
        try:
            ...
            if "returns" in train_perf.columns:
                ...
                logger.debug(
                    f"Calculated train returns from portfolio_value for {train_start} to {train_end}"
                )
            else:
                train_returns = pd.Series(dtype=float)
                logger.warning(
                    f"No returns data available for train period {train_start} to {train_end}"
                )
            ...
            if "returns" in test_perf.columns:
                ...
                logger.debug(
                    f"Calculated test returns from portfolio_value for {test_start} to {test_end}"
                )
            else:
                test_returns = pd.Series(dtype=float)
                logger.warning(
                    f"No returns data available for test period {test_start} to {test_end}"
                )
            ...
            print(
                f"  [{i + 1}/{len(combinations)}] Train {objective}: {train_obj:.4f}, Test {objective}: {test_obj:.4f}"
            )
    ...
    if len(results_df) > 0:
        best_idx = (
            results_df[f"test_{objective}"].idxmax()
            if f"test_{objective}" in results_df.columns
            else 0
        )
