...
def _to_yaml_safe_value(value: Any) -> Any:
    """
    Convert common numpy/pandas scalar types to built-in Python types.

    This avoids YAML emitting python-specific tags (e.g., numpy scalar objects),
    keeping saved parameter files portable and `yaml.safe_load()` compatible.
    """
        ...
        for param_name in param_names:
            if param_name in best_row:
                set_nested_param(
                    best_params,
                    param_name,
                    _to_yaml_safe_value(best_row[param_name]),
                )
