...
def aggregate_to_daily(minute_df: pd.DataFrame) -> pd.DataFrame:
    """
    Aggregate minute bars to daily bars using pandas resample.

    Note: Direct pandas usage (v1.12.0+).
    Previously used lib.data.aggregation.aggregate_ohlcv (removed).
    ...
    """
    return minute_df.resample("1D").agg(
