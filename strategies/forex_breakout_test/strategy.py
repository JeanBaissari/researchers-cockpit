...
def compute_signals(context, data):
    ...
    # ========================================================================
    # FOREX BREAKOUT STRATEGY - v1.12.0 Direct Zipline/Pandas APIs Only
    # ========================================================================
    # Logic: Price breaks above/below bands (mean ± threshold*std) + ATR filter
    # - Long: price > upper_band AND atr > volatility_filter
    # - Short: price < lower_band AND atr > volatility_filter
    # - Flat: Otherwise
    # ========================================================================
    ...
    # Get lookback period and breakout parameters from context
    lookback = context.params.get("strategy", {}).get("lookback_period", 24)
    breakout_threshold = context.params.get("strategy", {}).get("breakout_threshold", 1.5)
    volatility_filter = context.params.get("strategy", {}).get("volatility_filter", 0.0001)

    # Get price history using direct Zipline API (NO load_bundle wrapper)
    # Request lookback + 1 to have current bar plus lookback history
    try:
        prices = data.history(context.asset, "close", lookback + 1, "1h")
        highs = data.history(context.asset, "high", lookback + 1, "1h")
        lows = data.history(context.asset, "low", lookback + 1, "1h")
    ...
    # Current price (most recent bar)
    current_price = prices.iloc[-1]

    # Calculate breakout bands using lookback period (exclude current bar)
    # Direct pandas operations (NO aggregate_ohlcv or other wrappers)
    historical_prices = prices.iloc[:-1]  # Last N bars excluding current
    mean_price = historical_prices.mean()
    std_price = historical_prices.std()

    upper_band = mean_price + (breakout_threshold * std_price)
    lower_band = mean_price - (breakout_threshold * std_price)

    # Calculate ATR for volatility filter (direct pandas operations)
    # True Range = max(high-low, abs(high-prev_close), abs(low-prev_close))
    # ATR = mean of True Range over lookback period
    prev_close = prices.shift(1)
    tr = pd.concat(
        [highs - lows, (highs - prev_close).abs(), (lows - prev_close).abs()], axis=1
    ).max(axis=1)

    # ATR over lookback period (exclude current bar)
    atr = tr.iloc[:-1].mean()

    # Generate breakout signal with volatility filter
    if atr > volatility_filter:
        ...
        signal = 0.0  # Insufficient volatility - filter out

    # Record additional data for analysis
    additional_data = {
        ...
        "volatility_filter_passed": atr > volatility_filter,
    }
    ...
    try:
        # Get historical prices using direct Zipline data.history() API
        ...
        # hourly_prices = minute_prices.resample('1h').last()  # Direct pandas
...
def analyze_multi_timeframe(context, data, lookback_minutes=1440):
    ...
    This is the CORRECT way to aggregate timeframes in v1.12.0+.
    ...
    Example Usage in compute_signals():
        ...
        if hourly_trend and daily_trend:
            signal = 1
    """
    # Get minute data using direct Zipline API
    minute_data = data.history(
        context.asset, ["open", "high", "low", "close", "volume"], lookback_minutes, "1m"
    )

    # Aggregate to different timeframes using DIRECT pandas
    # NO aggregate_ohlcv() wrapper - this is the v1.12.0+ pattern
