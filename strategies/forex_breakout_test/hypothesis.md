...
EURUSD exhibits volatility-filtered breakout patterns where price movements beyond recent ranges (>1.5 standard deviations) signal continuation in the breakout direction, particularly when accompanied by elevated Average True Range (ATR > 0.0001).
...
- **Institutional Order Flow**: Large forex participants execute orders in size that push price beyond ranges
- **Volatility Clustering**: High volatility periods persist, creating follow-through after breakouts
- **Stop Clusters**: Retail stop losses beyond ranges amplify breakout moves
- **Information Flow**: News/economic data releases cause sustained directional moves
...
- Medium to high volatility environments (ATR > 0.0001)
- News-driven markets (economic releases, central bank announcements)
- London/New York session overlap (high liquidity)

**Fails in:**
- Choppy, range-bound markets (low directional persistence)
- Very low volatility periods (ATR < 0.0001)
- Asian session (lower liquidity, more false breakouts)
- Major holiday periods (thin liquidity)
...
- If Sharpe < 0.3 across 2+ years of 1-hour data, the breakout edge doesn't exist
- If win rate < 35%, breakouts don't have enough follow-through
- If average loss > 2x average win, risk/reward is unacceptable
- If maximum drawdown > 25%, strategy is too risky for intraday forex
...
**TEST STRATEGY** - v1.12.0 Validation Purpose:
- Calculate 24-hour rolling mean and standard deviation
- Breakout bands: mean ± (1.5 × std)
- ATR volatility filter: minimum 0.0001 threshold
- Long on upper band break + ATR > threshold
- Short on lower band break + ATR > threshold
- 2% fixed stop loss

**v1.12.0 Direct API Usage (NO WRAPPERS):**
- `data.history()` for price data (NO load_bundle wrapper)
- `pandas.resample()` for aggregation (NO aggregate_ohlcv wrapper)
- `get_calendar('FOREX')` for calendar (NO SessionManager)
- `symbol(asset_symbol)` for asset reference
- `order_target_percent()` for order placement
