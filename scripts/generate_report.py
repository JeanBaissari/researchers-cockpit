...
@click.option(
    "--output", default=None, help="Output file path (default: reports/{strategy}_report_{date}.md)"
)
@click.option(
    "--asset-class",
    default=None,
...
@click.option(
    ...
    "update_catalog_flag",
    is_flag=True,
    help="Update strategy catalog after generating report",
)
@click.option(
    "--status",
    default="testing",
...
def main(strategy, result_type, output, asset_class, update_catalog_flag, status):
    ...
    with LogContext(phase="report_generation", strategy=strategy, result_type=result_type):
        ...
        try:
            ...
            click.echo(
                f"  Run backtest first: python scripts/run_backtest.py --strategy {strategy}",
                err=True,
            )
            sys.exit(1)
