...
def parse_param_range(param_str: str) -> tuple[str, list]:
    ...
    if len(parts) < 2:
        raise ValueError(
            f"Invalid parameter format: {param_str}. Expected 'name:range' or 'name:start:end:step'"
        )
    ...
    if len(range_parts) == 3:
        ...
        raise ValueError(
            f"Invalid range format. Expected 'name:start:end:step' or 'name:val1,val2,val3'"
        )
...
@click.option(
    "--train-pct", type=float, default=0.7, help="Training data percentage (default: 0.7)"
)
...
def main(
    strategy, method, params, start, end, objective, train_pct, n_iter, capital, bundle, asset_class
):
    """
    ...
    with LogContext(phase="optimization", strategy=strategy, method=method):
        ...
        try:
            ...
            if overfit_file.exists():
                ...
                logger.info(
                    f"Overfit analysis: efficiency={overfit.get('efficiency', 0):.2f}, pbo={overfit.get('pbo', 0):.2f}"
                )
                click.echo(f"\nOverfit Analysis:")
