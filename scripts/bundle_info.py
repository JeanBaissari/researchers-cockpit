...
def get_bundle_info(bundle_name: str, verbose: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get minute bar info
        if (
            hasattr(bundle_data, "equity_minute_bar_reader")
            and bundle_data.equity_minute_bar_reader is not None
        ):
            minute_reader = bundle_data.equity_minute_bar_reader

            # Try to get minute bar count
            if hasattr(minute_reader, "first_trading_day") and hasattr(
                minute_reader, "last_available_dt"
            ):
                try:
...
@click.option("--list", "-l", "list_bundles_flag", is_flag=True, help="List all available bundles")
...
def main(bundle_name: Optional[str], list_bundles_flag: bool, verbose: bool, output_json: bool):
    ...
    if not bundle_name:
        ...
        click.echo(
            "✗ Error: Please provide a bundle name or use --list to see available bundles.",
            err=True,
        )
