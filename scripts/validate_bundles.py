...
def validate_date_field(value: Any, field_name: str) -> tuple:
    ...
    if not isinstance(value, str):
        return (
            False,
            f"{field_name} should be string or null, got {type(value).__name__}",
            ("set_null", field_name),
        )
    ...
    if value.lower() in [tf.lower() for tf in VALID_TIMEFRAMES]:
        return (
            False,
            f"{field_name} contains timeframe '{value}' instead of date",
            ("set_null", field_name),
        )
    ...
    try:
        ...
        for fmt in ["%Y/%m/%d", "%d-%m-%Y", "%m/%d/%Y"]:
            try:
                ...
                return (
                    False,
                    f"{field_name} uses non-standard format: '{value}'",
                    ("set_date", field_name, correct_date),
                )
            except ValueError:
                continue
...
def validate_bundle_entry(bundle_name: str, meta: dict) -> list:
    ...
    if "symbols" in meta:
        ...
        if not isinstance(symbols, list):
            issues.append(
                ("invalid_type", f"symbols should be a list, got {type(symbols).__name__}", None)
            )
        elif len(symbols) == 0:
            ...
            for i, sym in enumerate(symbols):
                if not isinstance(sym, str):
                    issues.append(
                        (
                            "invalid_type",
                            f"symbols[{i}] should be string, got {type(sym).__name__}",
                            None,
                        )
                    )
                elif not sym.strip():
    ...
    if "calendar_name" in meta:
        ...
        if cal is None:
            ...
            issues.append(
                ("invalid_type", f"calendar_name should be string, got {type(cal).__name__}", None)
            )
            ...
            issues.append(
                (
                    "invalid_value",
                    f"Unknown calendar: '{cal}'{suggestion}. Valid: {', '.join(CUSTOM_CALENDAR_NAMES)} (custom) + standard exchange calendars",
                    None,
                )
            )
    ...
    if "data_frequency" in meta:
        ...
        if freq is None:
            ...
            issues.append(
                (
                    "invalid_type",
                    f"data_frequency should be string, got {type(freq).__name__}",
                    None,
                )
            )
...
@click.option(
    "--check-disk/--no-check-disk", default=True, help="Check if bundle data exists on disk"
)
def main(fix, bundle, verbose, check_disk):
    ...
    with LogContext(phase="bundle_validation", bundle=bundle or "all", fix_mode=fix):
        logger.info(
            f"Starting bundle validation (bundle={bundle}, fix={fix}, check_disk={check_disk})"
        )
        ...
        if bundle:
            if bundle not in registry:
                ...
                click.echo(
                    f"  Ingest bundle: python scripts/ingest_data.py --bundle-name {bundle}",
                    err=True,
                )
                sys.exit(1)
        ...
        click.echo(
            f"Using {len(CUSTOM_CALENDAR_NAMES)} custom calendars: {', '.join(CUSTOM_CALENDAR_NAMES)}"
        )
        ...
        # Summary
        logger.info(
            f"Validation complete: {len(bundles_with_issues)} bundles with issues, {total_issues} total issues"
        )
        click.echo("\n" + "=" * 50)
        ...
        if bundles_with_issues and not fix:
            fixable_count = sum(
                1
                for bn in bundles_with_issues
                for issue in validate_bundle_entry(
                    bn, original_registry.get(bn, registry.get(bn, {}))
                )
