...
def find_files_with_old_imports(root: Path) -> List[Path]:
    ...
    """
    pattern = re.compile(
        r"from\s+lib\.bundles\.csv_bundle\s+import|import\s+lib\.bundles\.csv_bundle"
    )
    files_to_update = []
...
def main():
    ...
    parser.add_argument(
        "--auto-fix", action="store_true", help="Automatically fix all imports without confirmation"
    )
