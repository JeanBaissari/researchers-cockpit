...
def parse_bundle_name(bundle_name: str) -> Dict[str, str]:
    ...
    v1.12.0+ Bundle naming convention: {symbol}_{timeframe}
    Examples:
        spy_daily -> symbol='spy', timeframe='daily'
        eurusd_1m -> symbol='eurusd', timeframe='1m'
        btcusd_1h -> symbol='btcusd', timeframe='1h'

    Legacy (v1.11.1 and earlier): {source}_{asset}_{timeframe}
    Examples:
    ...
    Returns:
        Dict with 'source', 'assets', 'symbol', and 'timeframe_from_name' keys
        For v1.12.0 bundles: 'symbol' and 'timeframe_from_name' are populated
        For legacy bundles: 'source', 'assets', and 'timeframe_from_name' are populated
    """
    parts = bundle_name.split("_")

    # v1.12.0+ convention: {symbol}_{timeframe} (typically 2 parts)
    if len(parts) == 2:
        # Assume v1.12.0 format: symbol_timeframe
        return {
            "symbol": parts[0],
            "timeframe_from_name": parts[1],
            "source": None,  # Not available in v1.12.0 format
            "assets": None,  # Not available in v1.12.0 format
        }

    # Legacy convention: {source}_{asset}_{timeframe} (3+ parts)
    if len(parts) >= 3:
        return {
            "source": parts[0],
            "assets": parts[1],
            "timeframe_from_name": "_".join(parts[2:]),  # Handle multi-part timeframes
            "symbol": None,  # Not available in legacy format
        }
...
@click.command()
@click.option(
    "--bundles", "-b", default=None, help="Comma-separated list of bundle names to re-ingest"
)
@click.option(
    "--timeframe",
    "-t",
...
@click.option(
    "--dry-run", is_flag=True, help="Show what would be re-ingested without actually doing it"
)
@click.option("--force", "-f", is_flag=True, help="Skip confirmation prompt")
@click.option("--list", "list_only", is_flag=True, help="List all bundles in registry and exit")
def main(bundles, timeframe, assets, source, dry_run, force, list_only):
    ...
    with LogContext(
        phase="reingest_all", timeframe=timeframe, assets=assets, source=source, dry_run=dry_run
    ):
        logger.info(
            f"Starting batch re-ingestion (filters: timeframe={timeframe}, assets={assets}, source={source})"
        )
    ...
    if bundles:
        ...
        if not force:
            click.confirm(f"\nProceed with re-ingesting {len(filtered)} bundle(s)?", abort=True)
