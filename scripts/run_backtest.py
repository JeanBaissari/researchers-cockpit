        ...
        try:
            ...
            try:
                ...
                if not is_valid:
                    ...
                    click.echo(
                        f"  Fix: Edit parameters.yaml in strategies/{asset_class or '*'}/{strategy}/",
                        err=True,
                    )
                    sys.exit(1)
            except FileNotFoundError as e:
                logger.error(
                    f"Strategy not found: {strategy} (asset_class={asset_class})", exc_info=True
                )
                click.echo(f"✗ Error: Strategy '{strategy}' not found", err=True)
                click.echo(f"  Searched in: strategies/{asset_class or '*'}/", err=True)
                click.echo(
                    f"  Create strategy: Copy strategies/_template/ to strategies/{asset_class or 'equities'}/{strategy}/",
                    err=True,
                )
                sys.exit(1)
            ...
            if data_frequency is None:
                ...
                if bundle_to_check and bundle_to_check in registry:
                    ...
                    logger.info(
                        f"Auto-detected data frequency: {detected_freq} from bundle {bundle_to_check}"
                    )
                    click.echo(
                        f"Auto-detected data frequency: {detected_freq} (from bundle {bundle_to_check}, timeframe: {detected_tf})"
                    )
