...
class TestDeprecatedFilesRemoved:
    ...
    def test_validate_wrapper_removed(self):
        ...
        # Note: lib/strategy_validation/ package should exist (renamed from validate)
        # lib/validation/ package also exists (for data validation)
        wrapper_file = project_root / "lib" / "validate.py"
        strategy_validation_package = project_root / "lib" / "strategy_validation"
        validation_package = project_root / "lib" / "validation"
        assert not wrapper_file.exists(), "lib/validate.py wrapper should be removed"
        assert strategy_validation_package.exists(), "lib/strategy_validation/ package should exist"
        assert validation_package.exists(), "lib/validation/ package should exist (data validation)"
...
class TestNoDeprecatedImportsInLib:
    ...
    def test_lib_init_has_no_fallbacks(self):
        ...
        assert "from .extension" not in content, "lib/__init__.py should not import from .extension"
