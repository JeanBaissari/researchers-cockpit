...
def test_very_short_backtest(project_root_path):
    ...
    try:
        create_strategy_from_template(
            name=strategy_name, asset_class="equities", asset_symbol="SPY"
        )
        ...
        result_dir = save_results(strategy_name=strategy_name, perf=perf, params=params)
...
def test_extreme_parameters():
    ...
    extreme_params = {
        "strategy": {"asset_symbol": "SPY", "rebalance_frequency": "daily"},
        "position_sizing": {
...
def test_single_trade_strategy():
    ...
    transactions = pd.DataFrame(
        {"sid": [1], "amount": [100], "price": [100.0], "commission": [0.0]},
        index=[pd.Timestamp("2020-01-01")],
    )

    # Add exit transaction
    exit_transaction = pd.DataFrame(
        {"sid": [1], "amount": [-100], "price": [110.0], "commission": [0.0]},
...
def test_missing_data_periods():
    ...
    dates = pd.date_range("2020-01-01", periods=10, freq="D")
    returns = pd.Series(
        [0.01, np.nan, 0.02, np.nan, 0.01, 0.02, np.nan, 0.01, 0.02, 0.01], index=dates
    )
...
def test_zero_volatility_metrics():
    ...
    assert metrics["sharpe"] == 0.0, (
        f"Sharpe should be 0.0 for zero volatility, got {metrics['sharpe']}"
    )
    assert metrics["sortino"] == 0.0, (
        f"Sortino should be 0.0 for zero volatility, got {metrics['sortino']}"
    )
    # annual_volatility is now in percentage format (0.0% = 0.0)
    assert metrics["annual_volatility"] == 0.0, (
        f"Volatility should be 0%, got {metrics['annual_volatility']}"
    )
...
def test_single_losing_trade_metrics():
    ...
    transactions = pd.DataFrame(
        {"sid": [1, 1], "amount": [100, -100], "price": [100.0, 90.0], "commission": [0.0, 0.0]},
    ...
    assert metrics["max_win"] == 0.0, (
        f"max_win should be 0.0 when no wins, got {metrics['max_win']}"
    )
    assert metrics["avg_win"] == 0.0, (
        f"avg_win should be 0.0 when no wins, got {metrics['avg_win']}"
    )
