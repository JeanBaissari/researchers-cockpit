...
from lib.validation import ValidationResult, ValidationCheck, ValidationSeverity
...
class TestValidationResultPassedProperty:
    ...
    def test_passed_after_merge(self):
        ...
        assert result1.passed is False, (
            "Merged result should be False if any merged result has errors"
        )
...
class TestValidationResultErrorChecksProperty:
    ...
    def test_error_checks_includes_failed_error_severity(self):
        ...
        assert result.error_checks[0].severity == ValidationSeverity.ERROR, (
    ...
    def test_error_checks_multiple_errors(self):
        ...
        assert all(not c.passed for c in result.error_checks), "All checks should be failed"
...
class TestValidationResultWarningChecksProperty:
    ...
    def test_warning_checks_includes_failed_warning_severity(self):
        ...
        result = ValidationResult()
        result.add_check(
            "warning_check", False, "Warning message", severity=ValidationSeverity.WARNING
        )
        assert len(result.warning_checks) == 1, "Should have one warning check"
        assert result.warning_checks[0].name == "warning_check", (
            "Warning check should have correct name"
        )
        assert result.warning_checks[0].severity == ValidationSeverity.WARNING, (
    ...
    def test_warning_checks_multiple_warnings(self):
        ...
        assert all(not c.passed for c in result.warning_checks), "All checks should be failed"
...
class TestValidationResultPropertyInteractions:
    ...
    def test_properties_with_all_check_types(self):
        ...
        result.add_check(
            "passed_warning", True, "Passed warning", severity=ValidationSeverity.WARNING
        )
        result.add_check(
            "failed_warning", False, "Failed warning", severity=ValidationSeverity.WARNING
        )
        ...
        assert len(result.warning_checks) == 1, "Should have one warning check"
        assert result.warning_checks[0].name == "failed_warning", (
            "Should be the failed warning check"
        )
    ...
    def test_properties_consistency(self):
        ...
        assert warning_check_names == {"warning1"}, (
            "warning_checks should only have failed warnings"
        )
