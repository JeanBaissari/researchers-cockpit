...
def invalid_ohlcv_data_negative_prices():
    ...
    df = pd.DataFrame(
        {
            "open": [-10.0, 100.0, 101.0, 102.0, 103.0]
            + [100.0 + i * 0.1 for i in range(5, len(dates))],
            "high": [102.0 + i * 0.1 for i in range(len(dates))],
            "low": [99.0 + i * 0.1 for i in range(len(dates))],
            "close": [101.0 + i * 0.1 for i in range(len(dates))],
            "volume": [1000000 + i * 1000 for i in range(len(dates))],
...
class TestDataValidatorErrorDetection:
    ...
    @pytest.mark.unit
    def test_validation_errors_logged_missing_columns(
        self, caplog, invalid_ohlcv_data_missing_columns
    ):
        ...
        with caplog.at_level(logging.ERROR):
            result = validator.validate(
                invalid_ohlcv_data_missing_columns, asset_name="TEST_MISSING_COLS"
            )
    ...
    @pytest.mark.unit
    def test_validation_errors_logged_negative_prices(
        self, caplog, invalid_ohlcv_data_negative_prices
    ):
        ...
        with caplog.at_level(logging.ERROR):
            result = validator.validate(
                invalid_ohlcv_data_negative_prices, asset_name="TEST_NEGATIVE"
            )
    ...
    @pytest.mark.unit
    def test_validation_errors_logged_ohlc_inconsistency(
        self, caplog, invalid_ohlcv_data_ohlc_inconsistency
    ):
        ...
        with caplog.at_level(logging.ERROR):
            result = validator.validate(
                invalid_ohlcv_data_ohlc_inconsistency, asset_name="TEST_OHLC"
            )
    ...
    def test_validation_errors_logged_nulls(self, caplog, invalid_ohlcv_data_nulls):
        ...
        with caplog.at_level(logging.ERROR):
            result = validator.validate(invalid_ohlcv_data_nulls, asset_name="TEST_NULLS")
...
class TestValidationResultProperties:
    ...
    def test_validation_result_properties(self, valid_ohlcv_data):
        ...
        assert hasattr(result, "error_checks"), (
            "ValidationResult should have 'error_checks' property"
        )
        assert hasattr(result, "warning_checks"), (
            "ValidationResult should have 'warning_checks' property"
        )
