...
class TestConcurrentIngestion:
    ...
    def test_registered_bundles_set_concurrent_access(self):
        ...
        original_bundles = data_loader._registered_bundles.copy()
        ...
        def add_to_set(thread_id):
            try:
                ...
                data_loader._registered_bundles.add(bundle_name)
                time.sleep(0.01)  # Small delay to increase chance of race conditions
                assert bundle_name in data_loader._registered_bundles
            except Exception as e:
                errors.append((thread_id, str(e)))
        ...
        # Cleanup
        for i in range(num_threads):
            data_loader._registered_bundles.discard(f'concurrent_set_test_{i}')

        # Restore original state
        data_loader._registered_bundles = original_bundles
...
class TestCorruptedBundleRecovery:
    ...
    def test_is_valid_date_string(self):
        ...
        assert _is_valid_date_string('2020-12-31') is True

        # Invalid dates
        assert _is_valid_date_string('daily') is False  # Timeframe, not date
        assert _is_valid_date_string('invalid') is False
        assert _is_valid_date_string('') is False
        assert _is_valid_date_string(None) is False
        assert _is_valid_date_string('01-01-2024') is False  # Wrong format
        assert _is_valid_date_string('2024/01/01') is False  # Wrong separator
    ...
    def test_corrupted_end_date_detection(self, corrupted_registry_data):
        ...
        assert _is_valid_date_string(corrupted_entry['end_date']) is False, \
    ...
    def test_load_bundle_with_corrupted_registry(self, tmp_path):
        ...
        if end_date:
            is_valid = _is_valid_date_string(end_date)
