...
from lib.backtest import run_backtest, BacktestConfig
from lib.config import load_strategy_params, validate_strategy_params
from lib.strategies import check_and_fix_symlinks, get_strategy_path
...
def test_invalid_parameters():
    ...
    assert any("rebalance_frequency" in e.lower() for e in errors), (
        "Should error on invalid frequency"
    )
...
def test_invalid_position_sizing():
    ...
    assert not is_valid, "Parameters should be invalid"
    assert any("max_position_pct" in e.lower() for e in errors), (
        "Should error on invalid position size"
    )
...
def test_insufficient_data_handling():
    ...
    # Note: Strategy may not exist, which is also a valid error case
    ...
    # Error should mention date range, bundle coverage, or strategy not found
    # (strategy not found is also a valid error case)
    assert any(
        keyword in error_msg
        for keyword in ["date", "bundle", "range", "available", "strategy", "not found", "searched"]
    ), f"Error should mention date/bundle/strategy issue: {error_msg}"
