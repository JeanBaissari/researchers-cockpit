...
from lib.strategies import create_strategy_from_template, get_strategy_path
...
def test_end_to_end_workflow(
    project_root_path, test_strategy_name, test_asset_class, cleanup_test_strategy
):
    """
    ...
    strategy_path = create_strategy_from_template(
        name=strategy_name, asset_class=asset_class, asset_symbol="SPY"
    )
    ...
    # v1.12.0+: Bundle naming convention is {symbol}_{timeframe} (e.g., spy_daily)
    # Fallback to any available bundle if preferred bundle not found
    bundle = "spy_daily" if "spy_daily" in bundles else bundles[0]
    ...
    try:
        ...
        result_dir = save_results(
            strategy_name=strategy_name, perf=perf, params=params, result_type="backtest"
        )
...
@pytest.mark.integration
def test_strategy_creation_from_template(
    project_root_path, test_strategy_name, test_asset_class, cleanup_test_strategy
):
    ...
    assert strategy_results_link.resolve() == results_dir, (
        "Symlink should point to results directory"
    )
